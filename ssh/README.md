...
通过远程主机 A (ssh 可连接), 将本机的指定端口m映射到远程主机 B (可通过 A 放达) 的指定端口n上.
...
将一台远程主机 A 的指定端口 a, 经由本机, 映射到另一台远程主机 B 的指定端口 b 上

A 主机访问指定端口 a, 相当于访问 B 主机的指定端口 b
...
可以通过 SSH 主机来作为本地主机的代理, 让数据经由 SSH 主机和外部服务器进行交互
...
指定本地主机的端口通过 ssh 转发到指定远程主机. 格式: `LocalForward [bind_address:]post host:port`, 支持 IPv6.
